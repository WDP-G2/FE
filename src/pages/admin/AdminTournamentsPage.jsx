...
export default function AdminTournamentsPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        toast.error(
          getApiErrorMessage(err) || "Không tải được danh sách giải đấu",
        );
      } finally {
  ...
  return (
    ...
    >
      ...
      {loading ? (
        <div className="rounded-3xl border border-white/10 bg-white/[0.045] py-20 text-center text-white/50">
...
function StatusBadge({ status }) {
  ...
  return (
    <span
      className={`inline-flex rounded-full border px-4 py-2 text-sm font-semibold ${tones[status]}`}
    >
      {status}
    </span>
  );
    ...
    <article className="overflow-hidden rounded-3xl border border-white/10 bg-white/[0.045]">
      ...
      <div className="p-6">
        <div className="grid grid-cols-2 gap-x-5 gap-y-4 border-b border-white/10 pb-6 text-base text-white/60">
          ...
          <CardMeta
            icon={Users}
            text={`${tournament.registrationCount} đăng ký`}
          />
        </div>
