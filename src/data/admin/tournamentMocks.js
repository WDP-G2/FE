...
export function createRaces(prefix, counts, datePrefix = '2026-06', startDay = 10) {
  return counts.map((registered, index) => {
    const no = index + 1
    const finalName =
      no === counts.length ? 'Chung kết' : no === counts.length - 1 ? 'Bán kết' : `Vòng loại ${no}`
    return {
      id: `${prefix}-r${no}`,
      no,
      name: finalName,
...
export const detailTournaments = {
  ...
  'hanoi-cup-2025': {
    ...
    races: createRaces('hnc', [6, 6, 6, 5, 5, 5, 5, 4], '2025-12', 5).map((race) => ({
      ...race,
      status: 'Đã kết thúc',
    })),
  },
